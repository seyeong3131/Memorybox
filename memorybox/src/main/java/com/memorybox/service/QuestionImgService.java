...
public class QuestionImgService {
    ...
    public void saveQuestionImg(QuestionImg questionImg, MultipartFile questionImgFile) throws Exception {

        String oriImgName = questionImgFile.getOriginalFilename(); // 오리지날 이미지 경로
        String imgName = "";
        String imgUrl = "";
        ...
        questionImg.updateQuestionImg(oriImgName, imgName, imgUrl);
        questionImgRepository.save(questionImg);
    }

    public void updateQuestionImg(Long questionImgId, MultipartFile questionImgFile) throws Exception {
        if (!questionImgFile.isEmpty()) { // 상품의 이미지를 수정한 경우 상품 이미지 업데이트
            QuestionImg savedQuestionImg = questionImgRepository.findById(questionImgId).
                    orElseThrow(EntityNotFoundException::new); // 기존 엔티티 조회
            // 기존에 등록된 상품 이미지 파일이 있는경우 파일 삭제
            if (!StringUtils.isEmpty(savedQuestionImg.getImgName())) {
                fileService.deleteFile(itemImgLocation + "/" + savedQuestionImg.getImgName());
            }
            String oriImgName = questionImgFile.getOriginalFilename();
            String imgName = fileService.uploadFile(itemImgLocation, oriImgName,
                    questionImgFile.getBytes()); // 파일 업로드
            ...
            //※ 영속성 상태여야함 사용가능
            savedQuestionImg.updateQuestionImg(oriImgName, imgName, imgUrl);
        }
    }
}